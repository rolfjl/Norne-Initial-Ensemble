function NorneInitialEnsemble(ensembleSize, randomNumber)
...
N_FP = sum([13309, 53, 3, 8, 5]); % number of free parameters: multz, multflt, multregt, kwo+kwg, owc
N_S = 3; % number of static fields
ensemble = zeros(N_A*N_S + N_FP,N); % initialize ensemble
...
for I = 1:N
    ...
    A_MZ = A_MZ(:);
    N_A_MZ = sum(A_MZ);
    B = 1; % state vector start index
    E = N_A_MZ; % state vector end index
    V = ones(N_L,1);
    M_MZ = kron(M_M',V);
    S_MZ = kron(S_M',V);
    X = GaussianWithVariableParameters([D(1),D(2),6],M_MZ,S_MZ,C_MZ,C_S);
    ensemble(B:E,I) = X(A_MZ==1);
...
% Define bounds
G = ones(N_A,1);
F_LB = [norne.zLB*ones(N_A_MZ,1); norne.multfltLB*ones(N_MF,1); norne.multregtLB*ones(N_MR,1);...
    KR_LU(1,1)*ones(4,1); KR_LU(2,1)*ones(4,1); norne.owcLB];
F_UB = [norne.zUB*ones(N_A_MZ,1); norne.multfltUB*ones(N_MF,1); norne.multregtUB*ones(N_MR,1);...
    KR_LU(1,2)*ones(4,1); KR_LU(2,2)*ones(4,1); norne.owcUB];
S_LB = [norne.poroLB*G;norne.permxLB*G;norne.ntgLB*G];
S_UB = [norne.poroUB*G;norne.permxUB*G;norne.ntgUB*G];

% Adjust parameters with bounds
N_S_G = N_S*N_A;
ensemble(1:N_FP,:) = AdjustVariableWithInBounds(ensemble(1:N_FP,:),F_LB,F_UB);
ensemble(N_FP+1:N_FP+N_S_G,:) = AdjustVariableWithInBounds(ensemble(N_FP+1:N_FP+N_S_G,:),S_LB,S_UB); %#ok<*NASGU>

% Save the ensemble
save('NorneInitEns.mat', 'ensemble');
